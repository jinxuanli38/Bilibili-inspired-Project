...
class CreatorDanmuPagingSource(
...
) : PagingSource<Int, CreatorDanmuItem>() {

    private val service = RetrofitClient.create(UcenterService::class.java)

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CreatorDanmuItem> {
        return try {
            val page = params.key ?: 1
            val response = JSONObject(service.loadDanmu(pageNo = page, videoId = videoId))
            if (!response.isSuccess()) {
                return LoadResult.Error(Exception(response.errorMessage()))
            }
            val data = response.getJSONObject("data")
            val listArray = data.getJSONArray("list")
            val list = mutableListOf<CreatorDanmuItem>()
            for (i in 0 until listArray.length()) {
                val item = listArray.getJSONObject(i)
                list.add(
                    CreatorDanmuItem(
                        danmuId = item.optInt("danmuId"),
    ...
    override fun getRefreshKey(state: PagingState<Int, CreatorDanmuItem>): Int? =
